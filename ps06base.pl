...
sentence(TreeList,B) :- vp(TreeList,B,[]).
...
vp([vp,DP,VB],B,E) :- dp(DP,NUM,B,M), vb(VB,NUM,M,E).
vb([vb,IV],NUM,B,E) :- iv(IV,NUM,B,E).
vb([vb,TV,DP],NUM,B,E) :- tv(TV,NUM,B,M), dp(DP,_,M,E).

% 2 
vb([vb,VB,PP],NUM,B,E) :- vb(VB,NUM,B,M), pp(PP,M,E).

% dp([dp,DET,NP],NUM,B,E) :- det(DET,B,M), np(NP,NUM,M,E).
dp([dp,DET,NP],NUM,B,E) :- det(DET,NUM,B,M), np(NP,NUM,M,E).
...
% 3
nb([nb,NOUN,CP],NUM,B,E) :- noun(NOUN,NUM,B,M), cp(CP,M,E).

% 2
pp([pp,PREP,DP],B,E) :- prep(PREP,B,M), dp(DP,_,M,E).

% 3
rp([rp,REL],B,E) :- rel(REL,B,E).

% 3
cp([cp,RP,CB],B,E) :- rp(RP,B,M), cb(CB,M,E).
cb([cb,VP],B,E) :- vp(VP,B,E).
